...
export function useDragUtils(options: UseDragOptions = {}) {
    ...
    function updateLast(touch: Touch | undefined, x?: number, y?: number) {
        if (!touch) {
            return;
        }
    ...
    function calcOffset(touch?: Touch) {
    ...
    function bindTouchStart(e: TouchEvent) {
        if (enable && !enable()) {
            return;
        }
        if (dragStatus !== DragStatus.Idle) {
    ...
    function bindTouchMove(e: TouchEvent) {
    ...
    function bindTouchEnd(e: TouchEvent) {
        if (dragStatus === DragStatus.Idle) {
            return;
        }

        const { x, y } = calcOffset();
        updateLast(e.changedTouches[0], x, y);
